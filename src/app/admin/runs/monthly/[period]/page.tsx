...
export default async function MonthlyRunDetailPage({ params }: Props) {
  const { period } = await params;
  await requireAdminPageAccess();
  const db = requireDb();
  const { run, artifacts, tasks } = await listRunArtifacts(db, "MONTHLY", period);

  return (
    <div className="space-y-6">
      <section className="rounded-2xl border border-black/10 bg-white p-6">
        <h1 className="text-xl font-semibold text-black">Monthly run: {period}</h1>
        {run ? (
      ...
      <section className="rounded-2xl border border-black/10 bg-white p-6">
        <h2 className="text-lg font-semibold text-black">Monthly artifact bundle</h2>
        {artifacts.length === 0 ? (
          <p className="mt-2 text-sm text-black/60">No artifacts synced for this monthly period.</p>
        ) : (
