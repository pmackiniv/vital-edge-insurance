...
export default function MedicareAdvantageLeadPage() {
  ...
  const [countResult, setCountResult] = useState<TpmoCountsResponse | null>(null);
  const [error, setError] = useState("");
  const [success, setSuccess] = useState("");

  const scheduleHref = site.scheduleUrl || `tel:${site.phoneE164}`;

  const disclaimerReady = useMemo(
    () => Boolean(countResult?.ok && countResult.disclaimer),
    [countResult],
  );

  const handleCountLookup = async () => {
    setError("");
    setSuccess("");
    const safeZip = zip.replace(/\D/g, "").slice(0, 5);
    if (safeZip.length !== 5) {
      setError("Enter a valid 5-digit ZIP before computing TPMO counts.");
      return;
    }
    ...
    setCountResult(null);
    try {
      const response = await fetch("/api/tpmo/counts", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          context: {
            channel: "web",
            discussionStage: "marketing",
            callRecordingConsent: callRecordingConsent,
            beneficiaryInitiated: true,
          },
        }),
      });
      const data = (await response.json().catch(() => ({}))) as TpmoCountsResponse;
      setCountResult(data);
      if (!response.ok || !data.ok) {
        setError(data.error || "Unable to compute TPMO counts right now.");
      }
    } catch {
      setError("Unable to compute TPMO counts right now.");
    } finally {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!disclaimerReady) {
      setError("Compute TPMO counts first so we can display your ZIP-specific disclaimer.");
      return;
    }
    ...
    const enrichedMessage = [
      ...
      "MA owner-only routing: Patrick handles plan discussion/enrollment.",
    ]
      .filter(Boolean)
      .join("\n");
    ...
    try {
      ...
      if (!response.ok || !data.ok) {
        setError(data.error || "Unable to submit your lead right now.");
        return;
      }
      setSuccess(data.message || "Thanks. Patrick will handle your Medicare Advantage plan discussion.");
    } catch {
      setError("Unable to submit your lead right now.");
    } finally {
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-12">
      <h1 className="text-3xl font-semibold text-black">Medicare Advantage Lead Request</h1>
      <p className="mt-3 text-sm text-black/70">
        We’ll collect your ZIP and contact details, generate your required TPMO disclosure, and route MA plan discussion
        directly to Patrick.
      </p>

      <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
        Medicare Advantage plan discussions and enrollment are handled by Patrick only.
      </div>
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4 rounded-2xl border border-black/10 bg-white p-6">
        ...
        <div className="flex flex-wrap gap-3">
          <button
            type="button"
            onClick={handleCountLookup}
            disabled={countLoading}
            className="rounded-xl border border-black/15 px-4 py-2 text-sm font-medium text-black/80 disabled:opacity-50"
          >
            {countLoading ? "Computing..." : "Compute TPMO counts"}
          </button>
          <button
            type="submit"
          ...
          >
            {submitLoading ? "Submitting..." : "Submit MA lead"}
          </button>
        ...
        {countResult?.ok && countResult.disclaimer ? (
          <div data-testid="tpmo-disclaimer" className="rounded-xl border border-black/10 bg-black/[0.02] p-3 text-xs text-black/80">
            <p className="font-semibold text-black">ZIP-based TPMO disclaimer</p>
            <p className="mt-2">{countResult.disclaimer}</p>
          </div>
        ) : null}
