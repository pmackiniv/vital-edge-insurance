...
export default function MedigapLeadPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!firstName || !lastName || !phone || !zip || !consent || !dataSharingConsent || !leadTransferDisclosureAck) {
      setError("Complete all required fields and disclosures.");
      return;
    }
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-12">
      <h1 className="text-3xl font-semibold text-black">Medigap Lead Request</h1>
      <p className="mt-3 text-sm text-black/70">
        Share your ZIP and contact details so our team can follow up on Medicare Supplement options.
      </p>
      <p className="mt-2 text-xs text-black/60">
        For immediate help, call <a href={`tel:${site.phoneE164}`} className="underline">{site.phoneDisplay}</a>.
      </p>
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4 rounded-2xl border border-black/10 bg-white p-6">
        ...
        <label className="flex items-start gap-2 text-xs text-black/70">
          <input
            ...
            required
          />
          <span>I understand my request may be transferred to a licensed agent for follow-up.</span>
        </label>
        ...
        >
          {submitting ? "Submitting..." : "Submit Medigap lead"}
        </button>
